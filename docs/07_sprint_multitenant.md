...
- [ ] **L10 (regroupe tous les noms/infos d'agence en dur hors navbar)** — cadrage 31/05. ✅ **Données déjà en base** (UPDATE fait, commit `b48d4b7`) : société `nom_societe`/`siret` 14ch/`rcs` + agence `nom`/`adresse`/`telephone`. **DÉCOUPAGE EN 3 SOUS-LOTS SÉQUENTIELS** (pas un gros patch fourre-tout — chaque sous-lot = 1 branche + 1 test) :
  - **L10-a (facile, gros gain)** : `parametres/page.js` bloc Agence (l.275-290) dynamisé (raison sociale, franchise, SIRET, RCS, adresse, tél lus depuis `societes`+`agences` via `useAuth().societe_id` — page admin authentifiée, fetch dédié car plusieurs colonnes ; garder lecture seule + message contact `anne-lise.caillet@outlook.com`) + **retrait du texte « Modifiable dans Sécurité via Supabase Auth »** (l.257, bloc Profil franchisée) + **`login/page.js:61` neutralisé** : retirer « Martigues » → login générique BATILIS (pas d'utilisateur connecté = aucune source fiable d'agence ; afficher une agence en dur est incohérent en multi-tenant). Répartition niveaux : infos légales (raison sociale/SIRET/RCS) = SOCIÉTÉ ; adresse/tél/libellé franchise = AGENCE. ⚠️ multi-agence = plusieurs libellés franchise.
  - **L10-b (moyen, isolé)** : `api/cron/relances/route.js:83` — `AGENCY_CITY` env + signatures « illiCO travaux » → lire `dossier.agence_id → agences(nom,ville)` PAR DOSSIER (bon design multi-agence). ⚠️ boucle décennale artisan (l.371-393) = pas de dossier, pas d'agence claire (artisan société-wide) → fallback à définir.
...
**Prochaine action** : **L10-a** — `parametres` bloc Agence dynamisé + retrait texte profil « Modifiable dans Sécurité... » + `login` neutralisé (retirer « Martigues »). Sous-lot facile, faible risque, gros gain visible. Voir case L10 pour le détail des 3 sous-lots. Puis L10-b (cron), L10-c (PDFs + tri éditorial avec Anne-Lise), puis écrans saisie objectifs. Espace-client reporté après fix connexion (L14a). Puis Phase 5 (L14a, BLOQUANT test).

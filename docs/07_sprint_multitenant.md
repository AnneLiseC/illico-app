...
- [ ] **L10 (regroupe tous les noms/infos d'agence en dur hors navbar)** — cadrage 31/05. ✅ **Données déjà en base** (UPDATE fait, commit `b48d4b7`) : société `nom_societe`/`siret` 14ch/`rcs` + agence `nom`/`adresse`/`telephone`. **DÉCOUPAGE EN 3 SOUS-LOTS SÉQUENTIELS** (pas un gros patch fourre-tout — chaque sous-lot = 1 branche + 1 test) :
  ...
  - **L10-b (moyen, isolé)** — cadrage 31/05, audit `d695499` (cron = 7 boucles, service_role, signatures centralisées à 90% via `signatureHtml(referente)`) :
    - **Boucles « par dossier » (1 devis artisan, 3 acompte, 4 facture finale, 5 RDV, 7 CR)** : élargir leur `select` PostgREST pour remonter `dossier.agence_id → agences(nom, ville)`. Cible concrète = **l.250 boucle 3** (`agenceVille` = `process.env.AGENCY_CITY || 'Martigues'` en dur → `agences.ville` du dossier). La signature reste la **référente** (déjà dynamique via `signatureHtml`). Boucle 2 = notif interne (pas d'email) → ignorer.
    - **Boucle 6 (décennale artisan)** : artisan société-wide (pas d'agence_id, pas de dossier, pas de référente). DÉCISION : **l'admin de la société signe** (`from: admin.email`, remonté via `artisan.societe_id → profiles WHERE role='admin'` ; une société = UN admin). Signature = « L'équipe illiCO travaux » + **liste des villes** des agences de la société (`agences.ville` jointes par " - "). Mono-agence → « L'équipe illiCO travaux Martigues ». (« illiCO travaux » en dur = OK, c'est la marque franchiseur, pas une donnée d'agence variable.) Remplace la signature inline en dur l.388.
    - **Fallback l.54** de `signatureHtml` (référente absente, rare) : laisser un générique propre ou dynamiser a minima.
    - 📝 **À revoir en L15** : le format d'affichage de la liste de villes quand une société aura PLUSIEURS agences (« …Martigues - Salon - Aix » : liste à rallonge à valider visuellement ; Anne-Lise pourrait changer d'avis en voyant le rendu réel multi-agences). Le code liste déjà les villes → techniquement multi-agence-compatible.
  - **L10-c (lourd, tri éditorial avec Anne-Lise)** : PDFs `api/pdf/route.js` (~6 occ.) + `api/pdf/restitution.js` (~12 occ. + ⚠️ « Marine MICHELANGELI » l.39 nom propre §3 + page de garde CTP l.269-271). Source : `dossier.agence_id → agences/societes`. ⚠️⚠️ **TRI ÉDITORIAL OBLIGATOIRE avec Anne-Lise** occurrence par occurrence : distinguer « illiCO travaux Martigues » (= agence locale, À DYNAMISER) de « Honoraires illiCO travaux » / royalties / slogan « pensez illiCO » (= branding FRANCHISEUR, NE PAS TOUCHER). + threading de l'objet agence dans sous-composants Hdr/Ftr/makeCoverPage. Claude liste les occurrences → Anne-Lise tranche chacune → patch précis.
...
**Prochaine action** : **L10-b** — `api/cron/relances/route.js` (cadrage acté, voir case L10-b). Boucles par dossier (cible l.250 boucle 3) : remplacer `agenceVille`/'Martigues' par `dossier.agence_id → agences.ville`. Boucle 6 décennale : l'**admin de la société signe** (`from: admin.email` via `artisan.societe_id → profiles role=admin`) + signature « L'équipe illiCO travaux » + liste des villes des agences (`agences.ville` jointes par " - "). Robustesse cron (try/catch par boucle, ne pas planter). Puis L10-c (PDFs + tri éditorial avec Anne-Lise), puis écrans saisie objectifs. Espace-client reporté après fix connexion (L14a). Puis Phase 5 (L14a, BLOQUANT test).

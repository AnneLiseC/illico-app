...
- [x] **Filtres devis ✅ (03/06).** l.254/255/628 → `getActiveDevis`/`getSignedDevis`. Neutre (ensembles identiques, test Jadras/Eppinger). Corrige le bug latent en_attente (le « prévisionnel devis non signés invisibles » noté avant était en fait le filtre `!== refuse`). Branche `feat/rebranch-finance-filtres-royalties`.
  ...
  - ⬜ RESTE sur page finance : double comptage AMO (lot dédié, vrai bug — voir Dettes) + `chargerTout` avale les erreurs (robustesse).
...
- [ ] **`chargerTout` (finances) avale les erreurs** : `Promise.all` ne récupère que `{ data }`, ignore `{ error }`, pas de `.catch` → si une requête échoue, écran à 0 € SILENCIEUX. Robustesse (pas un calcul) : ajouter la gestion d'erreur. À caser quand pratique.

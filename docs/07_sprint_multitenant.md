...
- [ ] **L10 (regroupe tous les noms/infos d'agence en dur hors navbar)** — cadrage 31/05. ✅ **Données déjà en base** (UPDATE fait, commit `b48d4b7`) : société `nom_societe`/`siret` 14ch/`rcs` + agence `nom`/`adresse`/`telephone`. **DÉCOUPAGE EN 3 SOUS-LOTS SÉQUENTIELS** (pas un gros patch fourre-tout — chaque sous-lot = 1 branche + 1 test) :
  ...
  - **L10-c (lourd) — TRI ÉDITORIAL TRANCHÉ avec Anne-Lise (31/05, audit `8663a06`). Découpé en c1 (route.js, simple) + c2 (restitution.js, threading lourd).**
    ...
    - **Technique** : `route.js` = simple (composants `RecapitulatifPDF`/`buildCRDocument` ont déjà `dossier` → enrichir le `select` du dossier avec agence+société, remplacer les littéraux). `restitution.js` = lourd (`Ftr` ~8 appels + `makeCoverPage` ~3 appels ne reçoivent PAS l'agence → threader l'objet agence/société ; `buildDossier*` ont `supabaseAdmin` donc fetch possible). makeCoverPage : raison sociale + adresse + CP + ville dynamiques.
...
**Prochaine action** : **L10-c1 — PDFs `route.js`** (le simple). Tri éditorial déjà tranché (voir case L10-c). Enrichir le `select` du dossier (POST l.477-481) avec `agence:agences(nom,ville,adresse,code_postal,telephone, societe:societes(nom_societe,siret,rcs))` ; dynamiser header l.146 + footer l.319 + footer CR l.456 (nom agence) ; remplacer fallback mort « Marine MICHELANGELI » l.340 par neutre. NE PAS toucher : frais conso (178/188), Honoraires illiCO (256/269/286). Tester en générant un récap + un CR. Puis **L10-c2** (restitution.js, threading Ftr/makeCoverPage). Puis écrans saisie objectifs. Espace-client reporté après fix connexion (L14a). Puis Phase 5 (L14a, BLOQUANT test).

...
| — | **PHASE 3 COMPLÈTE** (hors notifications reportée Phase 6) → place à la Phase 4 | ✅✅✅ |
| — | notifications (INSERT resserré) → reporté Phase 6 avec Messagerie | ⏳ Phase 6 |

**Prochaine action** : **PHASE 3 TERMINÉE** (toute la bascule RLS + L8 référence + masquage toggle CTP ; seul reste reporté = notifications INSERT, Phase 6). → Entrer en **PHASE 4 (adaptation applicative légère)** : L6 (AuthProvider + requireUser élargis agence_id/societe_id, nettoie la dette L6-light), L7 (navbar mono-agence), L10 (info agence dynamique dans params + PDFs), écrans de saisie objectifs CA (⚠️ dépendance : fournir agence_id à l'INSERT objectifs). Puis **PHASE 5** : L14a fix reconnexion (BLOQUANT pour le test). ⚠️ Avant d'attaquer la Phase 4, m-merger les branches de code en attente sur main : `claude/L8-trigger-reference` (L8) + `claude/laughing-mccarthy-oYec4` (toggle CTP).

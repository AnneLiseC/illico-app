...
- [ ] L7 Navbar mono-agence. **Audit e149c06** : BATILIS (rebranding L9) FAIT, en dur partout = cohérent (nom du produit SaaS, on n'y touche pas). Nom d'agence « Martigues » EN DUR dans `navbar.js:149` (`Martigues · {prenom}`), jamais lu depuis `agences`. Header alimenté par `useAuth()` seul (a `agence_id`/`societe_id` mais PAS le nom). `agences.nom` existe (« illiCO travaux Martigues »). **PÉRIMÈTRE DÉCIDÉ (31/05)** :
  - **L7 = `navbar.js` UNIQUEMENT** (le header principal). Rendre le « Martigues » dynamique.
  - **Logique (multi-agence-ready)** : agente → nom de SON agence (`agences.nom` via `profile.agence_id`) ; admin (`agence_id` NULL) → nom de SA société (`societes.nom_societe` via `profile.societe_id`). PAS de déduction « unique agence » (piège mono-agence). Le sélecteur d'agence admin = L15.
  - **Source : enrichir l'AuthProvider/`useAuth()`** pour charger UNE FOIS et exposer le nom à afficher (agence pour agente, société pour admin) → évite que navbar + L10 + espace-client refassent chacun un fetch. Petit ajout au contexte (transverse), rentable car réutilisé par L10.
- [ ] **L10 (regroupe tous les noms d'agence en dur hors navbar)** : `parametres/page.js:277-289` (bloc Agence statique COMPLET : raison sociale, franchise, SIRET, RCS, adresse, tél + ⚠️ email `parametres:289` = nom propre en dur, contrevient règle §3 → à dynamiser) + `espace-client/page.js:253` (« illiCO travaux Martigues ») + `login/page.js:61` (« Martigues ») + `api/cron/relances/route.js:83` (email, `AGENCY_CITY` env → table) + **les PDFs** (`route.js`/`restitution.js` dont fallback « Marine MICHELANGELI » l.39). Tous lus dynamiquement depuis `agences`/`societes` (réutilise l'enrichissement useAuth de L7 côté UI ; côté API/PDF/cron, fetch direct par societe_id/agence_id). Même nature, même source → un seul lot.
...
**Prochaine action** : **L10 — Info agence dynamique** (tous les noms d'agence/société en dur hors navbar). Voir la case L10 ci-dessous : `parametres` (bloc Agence complet + email), `espace-client`, `login`, `cron relances`, et les PDFs. ⚠️ Audit d'abord (confirmer les emplacements + d'où chaque endroit peut lire `agences`/`societes`). Côté UI (espace-client, login si dans le contexte) → réutiliser `displayAgenceName`/le contexte ; mais ⚠️ `parametres` a besoin de PLUS que le nom (SIRET, RCS, adresse, tél → plusieurs colonnes de `agences`/`societes`) donc fetch dédié ; côté API/PDF/cron → fetch direct par societe_id/agence_id (pas de contexte React). Puis écrans saisie objectifs. Puis Phase 5 (L14a reconnexion, BLOQUANT test).

**Note de méthode** : la découverte du blocage NOT NULL/INSERTs en fin de Phase 2 confirme la valeur du protocole « tester après chaque MT ». Si on avait enchaîné directement Phase 3 sans test post-MT3, on aurait découvert le bug en plein milieu des RLS (bien pire à diagnostiquer). Erreur de planification partagée (binôme), détectée à temps par la méthode. Idem MT5 : le blocage RLS lecture `agences` a été révélé par le test des 9 chemins, pas deviné.lé par le test des 9 chemins, pas deviné.
...
- [ ] **L10 (regroupe tous les noms/infos d'agence en dur hors navbar)** — cadrage 31/05 :
  - **Données d'abord (UPDATE en base, fait avant la dynamisation)** : renseigner les infos CTP/Martigues (NULL en base depuis MT2). Mapping par niveau : **société** (`societes`) = `nom_societe` « CONSEIL TRAVAUX PROVENCE - CTP », `siret` « 94809688800011 » (14 ch. = vrai SIRET, corrige le SIREN 9 ch. de MT2), `rcs` « Aix-en-Provence 16/01/2023 » ; **agence** (`agences`) = `nom`/libellé franchise « illiCO travaux Martigues » (déjà OK, = `displayAgenceName` de L7), `adresse` « 22 RUE RAMADE, 13500 MARTIGUES », `telephone` « 0659810681 ». Toutes colonnes existent déjà (pas de migration).
  - **Répartition société vs agence** : infos légales (raison sociale, SIRET, RCS) = niveau SOCIÉTÉ (commun à toutes les agences) ; adresse, téléphone, libellé franchise = niveau AGENCE (propre à chaque agence). ⚠️ Multi-agence : plusieurs agences = plusieurs libellés franchise + adresses/tél (chacun sur sa ligne `agences`).
  - **Dynamiser l'affichage** (lire base au lieu d'en dur) : `parametres/page.js:277-289` (bloc Agence : raison sociale, franchise, SIRET, RCS, adresse, tél — lecture seule + message « contactez Anne-Lise à anne-lise.caillet@outlook.com » pour modif) + `espace-client/page.js:253` + `login/page.js:61` + `api/cron/relances/route.js:83` (email) + **PDFs** (`route.js`/`restitution.js`, fallback « Marine MICHELANGELI » l.39). Côté UI authentifiée → réutiliser le contexte `useAuth()` (L7) si suffisant, sinon fetch dédié (le bloc params a besoin de SIRET/RCS/adresse, plusieurs colonnes → fetch dédié societe+agence). Côté API/PDF/cron → fetch direct par societe_id/agence_id.
  - **Profil franchisée (params)** : Prénom/Nom/Téléphone éditables ; Email géré par Supabase Auth → **retirer le texte « Modifiable dans Sécurité via Supabase Auth »**.
  - ⚠️ **HORS L10 → reporté L14b** : l'auto-remplissage SIRET→raison sociale/RCS via API État (inutile pour le test, CTP renseignée en dur ici).

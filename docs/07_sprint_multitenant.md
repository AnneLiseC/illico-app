...
- [ ] **Dette désynchro `suivi_financier` frais ↔ dossiers.frais_statut** (découverte 03/06). Les lignes `suivi_financier` de type `frais_consultation` portent un `statut_client` désynchronisé de la VRAIE source `dossiers.frais_statut` (que l'app lit pour l'affichage). Inerte aujourd'hui (montant NULL, statut non lu), mais PIÈGE pour les audits (a fait croire que des frais réglés étaient « en attente »). Concernés : Souchon, Carmona, Gergaud. À assainir un jour (supprimer ces doublons inertes, ou synchroniser), pas urgent.
- [ ] **Dette comportement : décocher un toggle ne supprime PAS la ligne `suivi_financier`** (03/06). `majSuiviChantier`/`upsertOne` créent la ligne au 1er clic (INSERT avec `montant_ttc`) ; décocher repasse juste `statut_client='en_attente'`, la ligne et son montant figé RESTENT. Chaque manipulation de toggle laisse un résidu non réglé (montant figé qui fausse l'avancement si non-NULL, inerte si NULL). ⚠️ Le testeur en créera aussi. À traiter : supprimer la ligne quand on décoche la dernière échéance, ou ne créer qu'au règlement réel. Non bloquant mais pollue.
- [ ] **Migration `suivi_financier.montant_ttc` déjà persistés** (audité, sujet distinct) : impact réel ~5€ (Eppinger acompte_amo réglée, dérive devis). Lignes figées (montant_ttc INSERT-only). À décider : laisser (négligeable) ou rafraîchir au cas par cas. Faible priorité.
- [ ] **⚠️ Rappel : statut DEVIS** (recu/accepte/refuse/a_modifier/en_attente) **≠ statut DOSSIER** (devis_en_attente/… préfixés `devis_`) — ne pas confondre lors des corrections de filtres.

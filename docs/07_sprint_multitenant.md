...
**Prochaine action** : **L10-b** — `api/cron/relances/route.js` : remplacer `AGENCY_CITY` env + signatures « illiCO travaux » par lecture `dossier.agence_id → agences(nom, ville)` PAR DOSSIER (bon design multi-agence). ⚠️ Audit d'abord (structure exacte du cron, comment il itère sur les dossiers, où sont les mentions). ⚠️ boucle décennale artisan (l.371-393) = pas de dossier donc pas d'agence claire → fallback à définir (artisan société-wide). Sous-lot moyen, isolé. Puis L10-c (PDFs + tri éditorial avec Anne-Lise), puis écrans saisie objectifs. Espace-client reporté après fix connexion (L14a). Puis Phase 5 (L14a, BLOQUANT test).

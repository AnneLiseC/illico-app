...
| — | L5b (opérationnelles) + L5c (transverses) + L8 (réf chantier par agence) | ⏳ prochaine action |

**Prochaine action** : **L5b — RLS tables opérationnelles** : rendez_vous, interventions_artisans, photos, comptes_rendus, chantier_documents, chantier_fiches_techniques. Ce sont des FILLES de dossiers → même approche que les filles finances (`EXISTS dossiers`, validée en L5a-3). ⚠️ **Sujet à creuser : calendrier Google PARTAGÉ entre agences** (même lien pour Martigues, autres agences inconnu) = fuite potentielle multi-tenant au niveau du planning (table rendez_vous + intégration Google). Non critique pour le test (planning peut casser côté app), mais à documenter comme risque. Méthode : table par table, contrôle avant/après + smoke test, rollback.

Puis : **lot code « masquer onglets Agence/CTP côté agente »** (bug CTP = affichage confirmé). Puis L5c (transverses : messages, objectifs_ca, notifications INSERT resserré, fiches_techniques, specialites, artisans_specialites, google_tokens), L8 (référence chantier par agence).

...
- [ ] **L10 (regroupe tous les noms/infos d'agence en dur hors navbar)** — cadrage 31/05. ✅ **Données déjà en base** (UPDATE fait, commit `b48d4b7`) : société `nom_societe`/`siret` 14ch/`rcs` + agence `nom`/`adresse`/`telephone`. **DÉCOUPAGE EN 3 SOUS-LOTS SÉQUENTIELS** (pas un gros patch fourre-tout — chaque sous-lot = 1 branche + 1 test) :
  ...
  - **L10-c (lourd) — TRI ÉDITORIAL TRANCHÉ avec Anne-Lise (31/05, audit `8663a06`). Découpé en c1 (route.js, simple) ✅ FAIT + c2 (restitution.js, threading lourd) à faire.**
    ...
    - **Technique** : `route.js` ✅ **FAIT** (commit `3ad345c`, mergé `7d7bd20`) — dossier chargé enrichi avec `agence:agences!dossiers_agence_id_fkey(nom,ville,adresse,code_postal,telephone, societe:societes(nom_societe,siret,rcs))` (société chargée d'avance pour c2) ; dynamisé header l.146 + footer l.319 + footer CR l.456 (`dossier.agence?.nom`, fallback chaîne vide) ; fallback mort l.340 → `''` ; frais conso + honoraires intacts ; testé (récap + CR). **`restitution.js` = L10-c2 À FAIRE** : lourd (`Ftr` ~8 appels + `makeCoverPage` ~3 appels ne reçoivent PAS l'agence → threader l'objet agence/société ; `buildDossier*` ont `supabaseAdmin` donc fetch possible). makeCoverPage : raison sociale + adresse + CP + ville dynamiques. ⚠️ restitution.js reçoit le `dossier` en paramètre depuis route.js → réutiliser le dossier déjà enrichi (agence/société) plutôt que refetcher.
...
| 01/06 | **[HORS SPRINT — demande Marine] Modals de saisie : plus de fermeture au clic extérieur** (DevisModal puis 5 autres : ModalShell interventions/CR, agente créer/suppression, RDV planning, ModaleChoixClient). Retrait handler overlay + stopPropagation. Échap ModalShell conservé. Visionneuses/quick menu volontairement NON touchés. | ✅ `9d70cb3` (devis) + `77d2202`→`dde285a` (reste), testé |
| 01/06 | **L10-c1** : PDF `route.js` — nom d'agence dynamique (header + footer récap + footer CR), dossier enrichi agence+société. Frais conso/honoraires intacts | ✅ `3ad345c`, mergé `7d7bd20`, testé (récap+CR) |
...
**Prochaine action** : **L10-c2 — PDF `restitution.js`** (le lourd, threading). Tri éditorial déjà tranché (voir case L10-c). `restitution.js` reçoit le `dossier` en paramètre depuis `route.js` → ⚠️ vérifier que route.js lui passe le dossier DÉJÀ ENRICHI (agence/société, fait en c1) ou si `buildDossier*` doit fetcher via `supabaseAdmin`. À dynamiser : footer `Ftr` l.110 (~8 appels à threader), page de garde `makeCoverPage` l.269-271 (raison sociale + adresse + CP/ville, ~3 appels à threader). Référent (nom l.39 + tél l.35-36) toujours rempli → afficher `ref`, neutraliser les valeurs mortes « Marine MICHELANGELI »/« 06 59 81 06 81 ». NE PAS toucher : frais conso (132/374/802), honoraires (418/422/435/858), slogan (111), prompt IA (535). ⚠️ Tester en générant une restitution complète (R3 + suivi si possible). Puis écrans saisie objectifs. Espace-client reporté après fix connexion (L14a). Puis Phase 5 (L14a, BLOQUANT test).

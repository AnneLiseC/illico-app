...
- [x] **Dette TVA `/1.1` : FAUX PROBLÈME (audit 03/06, ne PAS corriger).** Le doc disait « `finances/page.js:349` reconstitue le HT via /1.1 au lieu de /1.2 » → c'était un MAUVAIS diagnostic. `/1.1` reconstitue le HT depuis le TTC des devis (rénovation à TVA 10%, ratio réel mesuré 1,10–1,1045) → ≈ exact. Passer à `/1.2` sur-diviserait et CASSERAIT les dossiers à TVA 10%. De plus, sur un dossier réglé, `acompteAmoNet` s'annule dans `honReel` (`soldeAmoNet = amoNet − acompteAmoNet`) → le `/1.1` est sans effet. Ce n'est PAS la cause d'un honReel faux. ⚠️ NE PAS « corriger » le `/1.1`. Le vrai bug est le double comptage AMO (ci-dessous).
- **Dette `/1.1` finances/page.js:349** : initialement crue incohérente (/1.1 vs /1.2) → AUDIT 03/06 : faux problème, /1.1 ≈ exact (TVA 10%), ne PAS toucher. Vrai bug = double comptage AMO (voir Dettes).

- **Ordre** : Étape 4 (commission, neutre) → puis 5+6+7 ENSEMBLE (change argent réel + persistance + migration → session dédiée, tête reposée).
- [ ] **🔴 Page finance — DOUBLE COMPTAGE honoraires AMO (vrai bug, ARGENT RÉEL — audit 03/06).** Sur un dossier AMO, l'honoraire se règle en 2 tranches : **acompte AMO = la part courtage (6%)**, **solde AMO = la part AMO (%amo)** — confirmé métier. L'« acompte AMO » et le « courtage » sont donc LE MÊME encaissement, pas deux distincts (cohérent avec l'effet de bord majSuiviChantier l.1679 qui les synchronise). MAIS `honReel` (l.391+394) et l'agrégation F2 (l.602-623) les ADDITIONNENT (`courtNet + acompteAmoNet`) → double comptage de la part courtage en état intermédiaire. Exemple Stevan (6%/6%, base 19 261 €, acompte réglé/solde non) : honReel affiche **1 996,14 €** au lieu de **1 155,66 €** réellement encaissé (la part courtage seule). Touche gains/redevances/facturation. S'annule quand le solde est réglé (Eppinger OK). **Sémantique cible** : AMO → acompte réglé = `f.honoraires.courtage.net` ; +solde réglé = `+ f.honoraires.soldeAmo.net` ; JAMAIS `courtNet + acompteAmoNet` (même montant). ⚠️ Lot DÉDIÉ, lié à la migration suivi_financier, vérif preview — c'est de la facturation réelle.

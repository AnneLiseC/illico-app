...
**Prochaine action** : **L10-c — PDFs** (`api/pdf/route.js` ~6 occ. + `api/pdf/restitution.js` ~12 occ.). ✅ **Confirmé à faire** : les PDFs SONT utilisés dans l'app (générés et vus par le testeur), contrairement aux emails. C'est le lot le plus lourd. Méthode : (1) Claude liste TOUTES les occurrences « illiCO travaux »/« Martigues »/noms propres des 2 fichiers (audit) ; (2) **tri éditorial avec Anne-Lise occurrence par occurrence** : « illiCO travaux Martigues » (agence locale → DYNAMISER via `dossier.agence_id → agences/societes`) vs « Honoraires illiCO travaux »/royalties/slogan « pensez illiCO » (branding FRANCHISEUR → NE PAS TOUCHER) + « Marine MICHELANGELI » l.39 (nom propre → dynamiser via référente/admin) ; (3) patch précis + threading objet agence dans sous-composants Hdr/Ftr/makeCoverPage. ⚠️ Tester en générant un vrai PDF. Puis écrans saisie objectifs. Espace-client reporté après fix connexion (L14a). Puis Phase 5 (L14a, BLOQUANT test).

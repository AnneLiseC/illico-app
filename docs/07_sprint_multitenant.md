...
- [ ] L6 AuthProvider + requireUser élargis. **État réel (audit cca9d1c)** : `app/lib/auth-context.js` expose déjà `profile` COMPLET (`select('*')`) via `useAuth()` → `agence_id`/`societe_id` déjà présents mais IMPLICITES (pas un contrat nommé). `requireUser` (`api-auth.js`) déjà élargi explicitement (L6-light). **Reste donc, côté front uniquement** : (1) **dette de re-fetch** — `clients/nouveau` (l.41-46) et `artisans/nouveau` re-fetchaient leur propre profil via `from('profiles').select('*')` au lieu d'utiliser `useAuth()` ; ⚠️ `artisans/nouveau` déjà nettoyé en partie par MT7 (mais vérifier s'il lit encore le profil en direct vs contexte) ; faire passer `clients/nouveau` (et tout autre) par `useAuth()`. (2) Optionnel : nommer explicitement `agence_id`/`societe_id` dans le contrat exposé par `useAuth()` (clarté). **Pas de gros travail** : socle déjà là. ⚠️ NE PAS coder l'agence active multi-agence ici (c'est L15) ; là où une déduction admin→agence subsiste (clients/nouveau), la centraliser éventuellement en 1 helper avec TODO L15 plutôt que de la dupliquer.
...
**Prochaine action** : **L6 (réduit)** — voir la case L6 ci-dessous. Le socle d'auth est déjà là (AuthProvider expose `profile` complet, `requireUser` élargi). Reste surtout : faire passer `clients/nouveau` (et tout autre re-fetch de profil) par `useAuth()` au lieu de re-fetcher, et éventuellement nommer le contrat `agence_id`/`societe_id`. ⚠️ Audit ciblé d'abord (état exact de `clients/nouveau` et `artisans/nouveau` après MT7 : utilisent-ils `useAuth()` ou re-fetchent-ils ?). Puis L7 (navbar), L10 (info agence dynamique PDF/params), écrans saisie objectifs. Puis Phase 5 (L14a reconnexion, BLOQUANT test).

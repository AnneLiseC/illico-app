...
- [ ] L7 Navbar mono-agence (header BATILIS + nom agence depuis table `agences`, plus en dur). ⚠️ **Mono-agence pour le test, MAIS codé « multi-agence-ready »** (ne rien faire qui devra être défait en L15) : nom d'agence lu DYNAMIQUEMENT depuis la table (jamais en dur), structure de header qui permettra d'AJOUTER le sélecteur d'agence en L15 sans refonte. Le sélecteur d'agence + vue active/consolidée = L15 (Phase 6, non testable tant qu'il n'y a qu'une agence). Pour le test : agente → nom de son agence ; admin → nom société ou agence unique (cadrer au strict nécessaire, noter le cas admin multi-agence pour L15).
...
**Prochaine action** : **L7 — Navbar mono-agence** (header BATILIS + nom agence depuis la table `agences`, plus en dur). ⚠️ Audit d'abord : où le header/navbar affiche le nom de l'app et/ou de l'agence aujourd'hui (en dur « illiCO travaux » ? « BATILIS » ?), et d'où vient le nom à afficher. Le nom d'agence vient de `agences.nom` (lisible via MT5 + useAuth pour le contexte). ⚠️ Penser multi-agence : pour un admin (pas d'agence unique), qu'affiche le header ? (nom société ? « toutes agences » ?) — mais L7 est « mono-agence », donc cadrer le strict nécessaire pour le test, noter le cas admin multi-agence pour L15. Puis L10 (info agence dynamique PDF/params), écrans saisie objectifs. Puis Phase 5 (L14a reconnexion, BLOQUANT test).

⚠️ **CONSIGNE PROCESS** (suite au lot L6) : exiger de Claude Code une **branche NEUVE par lot** (ne jamais réutiliser une branche déjà mergée — brouille l'historique).

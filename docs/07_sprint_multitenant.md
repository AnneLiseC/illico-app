...
- [ ] **🔴 BUG UNICITÉ REDEVANCES (bloquant multi-agente) — action réduite à 1 DROP (audit cohérence 01/06).** La bonne contrainte `redevances_agente_annee_mois_unique UNIQUE (agente_id, annee, mois)` EXISTE DÉJÀ en base. MAIS l'ancienne buggy `redevances_mois_annee_key UNIQUE (mois, annee)` n'a jamais été droppée → c'est elle qui bloque encore (2 agentes/même mois = collision). **Action restante : `DROP CONSTRAINT redevances_mois_annee_key` UNIQUEMENT** (vérifier 0 collision avant, fichier SQL versionné + rollback). ⚠️ Les DEUX contraintes ont été ajoutées en base SANS fichier SQL versionné → régulariser dans `docs/sql/`.

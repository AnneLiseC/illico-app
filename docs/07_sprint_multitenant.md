...
| — | Lot code « masquer onglets Agence/CTP côté agente » (bug affichage) | ⏳ **dernière pièce Phase 3** |
| — | notifications (INSERT resserré) → reporté Phase 6 avec Messagerie | ⏳ Phase 6 |

**Prochaine action** : **Lot code « masquer onglets Agence/CTP côté agente »** — DERNIÈRE pièce de la Phase 3. Le bug CTP est un bug d'AFFICHAGE confirmé (pas RLS) : les onglets « Agence - encaissements bruts » / « CTP - Résultat net » sont visibles côté agente dans la page finances alors que la RLS cloisonne déjà au niveau base. → masquer ces onglets côté code pour le rôle agente. Audit d'abord : trouver où ces onglets sont rendus dans `finances/page.js`, conditionner leur affichage à `isAdmin`. Après ce lot → **PHASE 3 COMPLÈTE** (hors notifications reportée Phase 6).

Puis : **Phase 4** (L6 AuthProvider, L7 navbar, L10 infos agence dynamiques, écrans saisie objectifs) puis **Phase 5** (L14a fix reconnexion, BLOQUANT test).

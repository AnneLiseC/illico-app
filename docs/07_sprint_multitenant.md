...
- [ ] **🔴 BUG UNICITÉ REDEVANCES (bloquant multi-agente)** — découvert audit 4. `redevances` a `UNIQUE (mois, annee)` SANS `agente_id` ni `agence_id` → deux agentes ne peuvent PAS avoir de redevance le même mois (collision d'unicité). Marche aujourd'hui (1 seule agente porteuse) mais **CASSE dès la 2ᵉ agente**. Comme chaque agente a des redevances, c'est bloquant pour toute société testeuse à ≥2 agentes. Fix : remplacer la contrainte par `UNIQUE (agente_id, mois, annee)` (vérifier 0 collision avant, fichier SQL + rollback).
...
- [ ] **`objectifs_ca` : contrainte d'unicité à confirmer** (audit 4 n'a pas listé `pg_indexes` ; le doc décrit `(annee,cible[,agente_id])`, peut-être portée par un index partiel — à vérifier).

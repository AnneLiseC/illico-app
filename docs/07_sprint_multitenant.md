...
- [ ] **L20 — navigation du menu client cassée** (2 bugs dans le même menu « ⋮ » de la carte client). Diagnostic confirmé par capture : la page `/clients/[id]/modifier` **EXISTE et fonctionne** (formulaire « Modifier le client » OK) → ce n'est PAS une page manquante.
  - **(1) Bouton « Modifier »** du menu → 404 : la **route du lien est mauvaise** (la page cible existe). Fix = corriger la cible du lien. Audit d'abord : trouver la route réellement générée vs la bonne (`/clients/[id]/modifier`).
  - **(2) « Voir les dossiers »** → envoie sur `/chantiers` (liste globale) au lieu du/des dossier(s) du client. Cible voulue : **1 dossier → aller directement sur ce dossier** ; **N dossiers → aller sur `/clients/[id]`** (fiche client qui liste ses dossiers). + **renommer le libellé au singulier « Voir le dossier »** quand le client n'a qu'un dossier. ⚠️ Audit : vérifier que le **nombre de dossiers du client** est disponible là où le menu est rendu (la carte affiche déjà « DOSSIERS N », donc probablement oui) ; sinon le remonter.
  - NB : la modification depuis la fiche client elle-même marche déjà (bouton Enregistrer de `/clients/[id]/modifier`). Seul le **menu** est en cause.
...
> Tant que A n'est pas soldé, l'app reste « testable » mais PAS « ouvrable » à une 2ᵉ franchise. C'est la frontière entre le test et la prod multi-tenant.
- [ ] **L12 Storage cloisonné par agence** + migration fichiers + policies versionnées (**dette sécu prioritaire**, D13). Aujourd'hui un `dossier_id` peut fuiter par copie d'URL entre comptes (toléré en test à 1 agence, inacceptable dès 2 franchises). **LE point bloquant n°1 de l'ouverture réelle.**
- [ ] **P0-9-bis — cloisonner par agente les 7 tables opérationnelles** (`rendez_vous`, `interventions_artisans`, `photos`, `comptes_rendus`, `chantier_documents`, `chantier_fiches_techniques`, `messages`). Faille de même classe que P0-9, testée empiriquement (une agente lit en direct les données d'un dossier d'une autre). Critique en multi-agences. Pattern : `admin OR EXISTS(dossier WHERE referente_id=auth.uid())`. (Détail dans Doc 5.)
- [ ] **Durcissements RLS** : (a) `get_my_role()` exécutable par `anon` → restreindre ; (b) policy INSERT `notifications` `WITH CHECK (true)` {public} → resserrer (à faire avec la réactivation Messagerie, bloc F, car les notifs y sont liées ; auditer d'abord qui crée les notifs — service_role/cron probablement) ; (c) leaked password protection (nécessite **plan Supabase Pro**) ; (d) captcha auth (optionnel, seulement si inscription publique un jour — pas le modèle actuel).

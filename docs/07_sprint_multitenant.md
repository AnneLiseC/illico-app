...
**Prochaine action** : **Phase 5 — L14a, fix reconnexion après invitation** (route `/auth/set-password` + `redirectTo` + `updateUser({password})`). C'est LE bloquant du test utilisateur. Touche à l'authentification = sensible → session dédiée, audit lecture seule d'abord (route `/api/create-agente` côté invitation, flow OTP actuel, `redirectTo`), puis patch. La Phase 4 est close (L10 complet + écrans objectifs CA mergés `4cc2a6`).

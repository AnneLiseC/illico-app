...
- [ ] **« Mot de passe oublié » sur la page login (à câbler — POST-TEST).** Le mode `'forgot'` existe déjà en state dans `app/login/page.js` mais son câblage (`resetPasswordForEmail` + page de réinitialisation recevant un lien *recovery*) n'est PAS branché (confirmé à l'audit L14a). Lot distinct de L14a : le lien *recovery* est un flux DIFFÉRENT du lien *invite* (déjà traité), avec sa propre Redirect URL et son propre test. **Décision (01/06) : reporté post-test** — non bloquant pour le test (le testeur définit son mdp via L14a ; en cas de perte, reset manuel dans Supabase Auth → Users, contournement déjà en place). À faire avec la passe auth/login post-test. Réutilisera la page `/auth/set-password` (ou une jumelle) pour la saisie du nouveau mdp. **(Lot F / auth — post-test.)**

...
- [ ] **Dette résiduelle `montant_ttc`/540 dans le vivant.** `renderSuiviAgenteFinancier` (VIVANT) lit encore `montant_ttc || 540`. La suppression du code mort n'a PAS soldé toute la dette `montant_ttc`/540. Conséquence : le DROP de la colonne `redevances.montant_ttc` (prévu étape 5) reste bloqué tant que ce lecteur vivant existe → à basculer sur `montant_ht` avant le DROP. **(Lot B facturation / Lot E.)**
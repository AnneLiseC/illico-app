...
- **✅ SPRINT 11-16/07 (confort & complétude produit — « Niveau 2 »)**. Rattrapage backlog : **responsive/adaptation écran** (mise en page côte-à-côte selon la taille, fin du « queue leu-leu » — rédhibitoire pour un client sur tablette/mobile), **planning en temps réel**, **solde honoraire AMO en plusieurs paiements**, **choix client pro/particulier + formes juridiques (SCI/EURL/SARL)**, **honoraires dans les PDF de suivi de facturation**, **dashboard interactif**, **page Statistiques (réactivée)**, **contrat de prestation sorti de la modale → dossier « à vu »**, **archivage auto des dossiers terminés/annulés**, **historique des devis**, **avancement chantier** + refonte **dates de retard/attention/relance** (calcul & cadrage corrigés) + **barre d'avancement**, **chargement inter-pages accéléré**, **vérification des connexions au login**, **chiffrement des tokens Google** (résout la dette du bloc A), **fix périmètre à la connexion d'une cible**, **RDV BNI Miramas en double** + **« modifier/supprimer cet événement ou toute la série »**, **upload vidéos de chantier**, **photos annotées façon Archireport** dans le CR, **fichiers vocaux → CR (IA, Deepgram + Claude)**. Détail → journal (confirmé produit ; commit par item non journalisé au moment du fait).
- **✅ DRIVE ONEDRIVE — COMPLET (17/07, #383 + docs-artisans-lots)**. Miroir sortant Supabase→OneDrive de la référente (documents, photos, CR validés, devis par statut Reçus/Signés/Refusés move-aware), arborescence chantier/catégorie/artisan/lot, inbox de rattachement, rattrapage idempotent, pull cron. item_id connu AVANT poll. Détail → journal.
- **✅ FINANCES Facturation agentes — REPORT F1 + CLÔTURE (17-21/07, #384→#391)**. Modèle final report mois à mois : dû cumulé = composantes du mois + reste du mois précédent (report auto, sans bouton) ; 2 KPI (Gains à facturer F1 / À régler F2) ; bouton « Clôturer » (marqueur 🔒 Reporté). Fix redevance F2 dans la vue propre de l'agente (agenteActuelle résolu via profile). finance.js INTOUCHÉ, migration additive `factures_agente.cloture`. Détail → journal.
- **✅ AUTH ROBUSTE — 401 onglet resté ouvert RÉSOLU (21/07, #393 + apiFetch partout)**. Cause prouvée (timer auto-refresh Supabase suspendu sur onglet inactif → token expiré). 3 couches : refresh au retour d'onglet (AuthProvider → couvre DB + storage + /api/*) + refresh proactif authHeaders + helper `apiFetch` (retry-sur-401 + redirection /login|/login-client) généralisé à TOUS les appels /api/*. Détail → journal.
- **✅ RENOMMAGE UI « Compte-rendu » → « Rapport de visite » / « CR » → « RV » (21/07, raison assurance)**. Texte affiché + titres PDF + prompt IA. Technique inchangé (catégorie `compte_rendu`, table `comptes_rendus`, routes `/api/cr`, dossier OneDrive « Comptes rendus »). Anciens titres en base conservés. Branche rename/rapport-visite. Détail → journal.
- **✅ SPRINT 22-25/07 (Drive, Outlook, thème Batilis, légal, éditrice)**. **Google Drive** complet (arbo statut-buckets, push/move/pull multi-provider) ; calendrier **Outlook** (push + pull) ; renommage définitif → **Batilis** (deux tons bleu illiCO + teal) ; pages **Confidentialité/CGU/RGPD** ; fondation auth **éditrice/super-admin** + désactivation compte cascade ; reset mdp via boîte propre ; email système via Outlook OAuth délégué. Détail → journal.
- **✅ SPRINT 29/07-04/08 (perf, typologies ESTIMO/MERAD, honoraires AMO, responsive)**. Perf lazy (charts/calendar/modales, LCP) ; **typologie ESTIMO** (estimation → bascule chantier) + **MERAD** (courtage allégé R3, commission acompte) ; honoraires solde AMO en tranches statut Payé/attente + 1 facture PDF/ligne ; devis pré-rempli IA + « Devis signé » ; CR IA ancré sur devis signés + photos `[[photo:ID]]` ; responsive (grilles/formulaires) + planning redesign ; auth 401 chargement profil. Détail → journal.
- **✅ GROS SPRINT 12-18/08 — REFONTE CR/CHANTIER « visites → actions » + GANTT**. Nouveau système CR structuré (onglet Lots/sous-lots CRUD, visites→actions, photos annotées + checklist, aide IA actions), **Gantt intégré** (frappe-gantt : arbre éditable, drag dates, dépendances/flèches, export PDF, jours ouvrés/entreprise, l'IA pousse les dates), reprise anciens rapports (consolidation IA), diffusion CR par mail, export PDF configurable, dictée + audio, numérotation par type, **archivage douce + panneau Historique**, sécurité endpoints CR. + vue Cartes/Trier par, fiches techniques IA, date démarrage auto. Détail → journal (12-18/08).
- **✅ SPRINT 19-26/08 (audio, PV, câblage CR, finance, nettoyage — session Cowork, PR #479→#500)**. Audio .m4a dans les CR ; PDF visite plafond 6 photos retiré ; menu ⋮ Annuler/Ré-ouvrir ; **CR câblés** (auteur_id, visite_rdv_id +calendrier, action_cibles.intervenant_id, cr_presences) + **versionnage** (parent_cr_id/version) ; **spécialités artisan par IA depuis la décennale** ; libellés statut passé/futur ; **restitution** PV=1 devis/1 PV (pv_path), MERAD sans kbis CTP, dédup artisan ; **honoraires courtage/AMO en TTC** (le montant était déjà TTC) ; **nettoyage base** (DROPs vérifiés : 6 colonnes + 2 tables + 3 type_echeance). Détail → journal.
- **✅ GANTT — PLANNING ROGNÉ RÉSOLU + MINIATURES PHOTOS (27/08, branche fix/gantt-visuel)**. Cause : le shim responsive `img, svg, video, canvas { max-width: 100% }` (globals.css L962) rognait le SVG du gantt, qui porte une largeur fixe et **pas de viewBox** → seule la « Vue ajustée » s'affichait, Jours/Semaines/Mois et tout scroll horizontal ne montraient que du vide, **sans erreur console**. Fix `.gantt-container svg.gantt{max-width:none}` + `infinite_padding:false` (supprime le re-rendu à chaque cran de molette et le mois de vide avant la 1re barre) + fenêtre forcée sur [aujourd'hui ; aujourd'hui + 4 semaines]. + `signerPhotos` en tranches de 6 (fin des 544 du Storage sur les gros chantiers). PDF du planning NON impacté (chemin serveur @react-pdf indépendant, vérifié). Détail → journal.
...
> - 🟡 **RGPD — document RÉDIGÉ mais PAS opérationnel** : remplir les [...] (SIRET, adresse, durées de conservation) + relecture par la conseillère. Non opposable tant que non fait. (Le chiffrement des tokens Google, ancienne dette de ce bloc, est FAIT — voir sprint 11-16/07.)
...
> - 🟢 **Petits restes fonctionnels** : Messagerie AMO bug raison_sociale (si persiste), notif mail upload facture, idées futures. Voir bloc F. (Statistiques RÉACTIVÉES au sprint 11-16/07.)
...
- **Suppression en base = 5 vérifs OBLIGATOIRES avant tout DROP** : 0 donnée + 0 réf. code + 0 réf. fonction/vue/policy/trigger + 0 FK entrante + go explicite. S'il en manque une → lister comme candidat, NE PAS supprimer. « Vide » se prouve par COUNT exact, JAMAIS par pg_stats/null_frac (erreur passée le 27/08).
- **Honoraires courtage/AMO (finance.js)** : `honoraires.courtage.ttc`/`soldeAmo.ttc` = taux × travaux **TTC** = la valeur affichée PARTOUT (KPI, écritures suivi_financier, échéances « Autres », honoraire client) ; `.ht` = taux × travaux HT. Ne JAMAIS multiplier `.ttc` par 1,20 (double TVA). Libellés : « % des travaux · … TTC ».

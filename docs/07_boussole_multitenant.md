...
> **Statut : onboarding L14b complet + cloisonnement prouvé. Chantier MULTI-AGENCE en cours — Lot 1 (création d'agence) FAIT.** Phases 1-5 closes, app multi-tenant fonctionnelle et cloisonnée.
...
- [ ] **Lot 2 — Vue active + navbar bi-zone** (LE morceau architectural). État « agence courante » à créer de zéro (inexistant aujourd'hui — contexte/param de navigation + persistance), navbar bi-zone (onglets par agence + séparateur + nav consolidée), défaut = Consolidé, garde-fou « invisible si 1 agence ». AUCUN scoping d'écran dans ce lot — juste l'ossature de navigation.
- [ ] **Lot 3 — Sélecteurs de création** : remplacer les `.single()` de `api/create-agente` (sélecteur d'agence dans modale Nouvelle agente) et `clients/nouveau` (branche référente=admin) par un choix d'agence. Touche la création, indépendant de l'affichage.

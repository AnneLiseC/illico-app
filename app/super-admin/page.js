                      ...
                      {s.comptes.map(c => {
                        ...
                        return (
                          <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', fontSize: 13, padding: '6px 10px', background: 'var(--surface-2)', borderRadius: 8 }}>
                            ...
                            <span style={{ padding: '1px 8px', borderRadius: 99, fontSize:11, fontWeight: 700, background: c.role === 'admin' ? 'rgba(79,70,229,0.1)' : 'var(--ink-100)', color: c.role === 'admin' ? 'var(--brand-800)' : 'var(--ink-600)' }}>

...
// Email d'invitation Batilis (envoyé par NOUS via la boîte d'envoi, plus par
...
export function buildInvitationEmail({ prenom, nom, actionLink, role }) {
  ...
  const intro = estAdmin
    ? "Vous êtes invité·e à rejoindre <strong>Batilis</strong> en tant que franchisé·e. Créez votre accès pour configurer votre société et votre première agence."
    : "Vous êtes invité·e à rejoindre <strong>Batilis</strong>. Créez votre accès pour rejoindre votre espace agent."
  const cta = estAdmin ? 'Créer mon accès' : 'Rejoindre mon espace'

  const subject = estAdmin
    ? 'Batilis — Invitation franchisé·e'
    : 'Batilis — Invitation à rejoindre votre espace'

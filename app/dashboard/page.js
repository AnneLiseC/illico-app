          ...
          return (
            <g key={i}>
              ...
              <rect x={x + w + 4} y={H - padY - hReel} width={w} height={hReel} rx="3"
                fill={d.inProgress ? '#7ccdef' : '#6366f1'} style={{ transition: 'all 400ms ease' }} />
              {d.reel > 0 && (
                <text x={x + w + w / 2 + 4} y={H - padY - hReel - 5}
                  textAnchor="middle" fontSize="10" fontWeight="700" fill="#4f46e5">
...
function Pipeline({ dossiers, onOpen }) {
  ...
  const buckets = [
    { key: 'a_traiter', label: 'À traiter',            tone: '#6366f1', match: s => ['a_contacter','a_relancer'].includes(s) },
    ...
    async function loadData() {
      ...
      const [{ data: dos }, { data: obj }, { data: rdv }] = await Promise.all([
        supabase.from('dossiers').select(`
          ...
          honoraires_amo_taux, taux_courtage, typologie, created_at, archive, agence_id, date_demarrage_chantier, date_fin_chantier, contrat_signe, apporteur_actif,
          ...
          devis_artisans(id, montant_ht, montant_ttc, commission_pourcentage, statut, date_signature, hors_honoraires, artisan:artisans(id, entreprise)),
            ...
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative' }}>
                {rdvAujourdhuiScoped.map((a, i) => {
                  ...
                  const dotColor = tone === 'info' ? '#6366f1' : tone === 'ok' ? '#16a34a' : tone === 'warn' ? '#f59e0b' : '#94a3b8'
                  ...
                  return (
                    <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '60px 12px 1fr', gap: 14, alignItems: 'flex-start', paddingBottom: 14, position: 'relative' }}>
                      ...
                      <div style={{ position: 'relative', display: 'flex', justifyContent: 'center' }}>
                        <span style={{ width: 10, height: 10, borderRadius: 99, background: dotColor, marginTop: 6, boxShadow: '0 0 0 3px #fff, 0 0 0 4px rgba(99,102,241,0.18)', flexShrink: 0 }} />

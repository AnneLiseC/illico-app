...
function App() {
  ...
  const [chantierDetailId, setChantierDetailId] = React.useState(null);
  const [openClientId, setOpenClientId] = React.useState(null);
  const [openArtisanId, setOpenArtisanId] = React.useState(null);
  ...
  const breadcrumbs = {
    ...
    chantiers:'Chantiers',
    chantier_detail:'Fiche chantier',
    clients:'Clients',
    artisans:'Artisans',
    planning:'Planning',
    finances:'Finances',
  ...
  const handleOpenChantier = (id) => {
    setChantierDetailId(id);
    setRoute('chantier_detail');
  };
  const closeDetail = () => {
    setChantierDetailId(null);
    setRoute('chantiers');
  };
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <main style={{flex:1, padding:'28px 32px 60px', overflow:'auto', minHeight:0, display:'flex', flexDirection:'column'}}>
          ...
          {route === 'chantiers'    && <Chantiers initialId={openChantierId} onClearInitial={()=>setOpenChantierId(null)} onOpenDetail={handleOpenChantier}/>}
          {route === 'chantier_detail' && chantierDetailId && (() => {
            const dossier = DATA.DOSSIERS.find(x => x.id === chantierDetailId);
            return dossier ? <ChantierDetail dossier={dossier} onBack={closeDetail}/> : null;
          })()}
          {route === 'clients'      && <Clients onOpenClient={(id) => { setOpenClientId(id); setRoute('client_detail'); }}/>}
          {route === 'artisans'     && <Artisans onOpenArtisan={(id) => { setOpenArtisanId(id); setRoute('artisan_detail'); }}/>}
          ...
          {route === 'notifications'&& <Notifications onOpenChantier={handleOpenChantier}/>}
          {route === 'espace_client'&& <EspaceClient/>}
          {route === 'client_detail'&& <ClientDetail onBack={()=>setRoute('clients')}/>}
          {route === 'artisan_detail'&& <ArtisanDetail onBack={()=>setRoute('artisans')}/>}
        </main>
      </div>

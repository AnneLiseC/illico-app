...
function Chantiers({ initialId, onClearInitial, onOpenDetail }) {
    ...
    <div className="page-enter" style={{display:'flex',flexDirection:'column',gap:18, minHeight:0, flex:1}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:16,flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button className="btn btn-primary" onClick={()=>setShowNew(true)}><I.Plus size={16}/> Nouveau chantier</button>
        </div>
      </div>
      ...
      <div className="grid-list" style={{flex:1, minHeight:0}}>
        ...
        {selected ? <ChantierPreview d={selected} onOpenDetail={onOpenDetail}/> : <EmptyDetail/>}
      </div>
      {showNew && <NewChantierModal onClose={()=>setShowNew(false)}/>}
    </div>
  );
}
...
function ChantierPreview({ d, onOpenDetail }) {
    ...
    <div className="card" style={{padding:0, overflow:'hidden', display:'flex', flexDirection:'column', minHeight:0}}>
      ...
      <div style={{padding:'18px 22px 16px', borderBottom:'1px solid var(--ink-200)', position:'relative'}}>
        ...
        <div style={{display:'flex',gap:8,marginTop:14}}>
          <button className="btn btn-primary" style={{fontSize:12, padding:'6px 12px'}} onClick={()=> onOpenDetail && onOpenDetail(d.id)}><I.Eye size={14}/> Ouvrir dossier</button>
